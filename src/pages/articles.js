...
const MovingImage = ({ title, img, link = '' }) => {
    ...
    return (
        <Link href={link} target="_blank" onMouseMove={handleMouse} onMouseLeave={handleMouseLeave}>
            ...
            <FramerImage ref={imgRef} src={img} alt={title} className="w-96 h-auto hidden absolute rounded-lg z-10 md:!hidden" style={{ x, y }} initial={{ opacity: 0 }} whileInView={{ opacity: 1, duration: 0.2 }} />
        </Link>
    )
}
...
const Article = ({ img, title, date, link = "" }) => {
    return (
        <motion.li className="relative w-full p-4 my-4 rounded-xl flex items-center justify-between bg-light text-dark first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:bg-dark dark:border-light dark:text-light sm:flex-col" initial={{ y: 200 }} whileInView={{ y: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} viewport={{ once: true }}>
...
const FeaturedArticle = ({ img, title, time, summary, link = "" }) => {
    return (
        <li className="col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2xl relative dark:bg-dark dark:border-light">
            <div className="absolute top-0 -right-3 -z-10 w-[102%] h-[103%] bg-dark rounded-[2.5rem] rounded-br-3xl dark:bg-light" />
